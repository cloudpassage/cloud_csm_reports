...
class HaloAPICaller(object):
    ...
    def get_auth_token(self, url, args, kid, sec):
        ...
        try:
            ...
            if hasattr(e, 'reason'):
                Utility.log_stderr("Failed to connect [%s] to '%s'" % (e.reason, url))
            elif hasattr(e, 'code'):
                msg = self.get_http_status(e.code)
                Utility.log_stderr("Failed to authorize [%s] at '%s'" % (msg, url))
                data = e.read()
                if data:
                    Utility.log_stderr("Extra data: %s" % data)
                Utility.log_stderr("Likely cause: incorrect API keys, id=%s" % kid)
            else:
    ...
    def do_get_request(self, url, token):
        ...
        try:
            ...
            if hasattr(e, 'reason'):
                ...
                msg = self.get_http_status(e.code)
                Utility.log_stderr("Failed to fetch events [%s] from '%s'" % (msg, url))
                if e.code == 401:
                    auth_error = True
            else:
                Utility.log_stderr("Unknown error fetching '%s'" % url)
    ...
    def do_put_request(self, url, token, put_data):
        ...
        try:
            ...
            if hasattr(e, 'code'):
                msg = self.get_http_status(e.code)
                Utility.log_stderr("Failed to make request: [%s] from '%s'" % (msg, url))
                if e.code == 401:
                    auth_error = True
    ...
    def get_csp_findings_count(self):
        url = "%s:%d/%s/csp_findings?sort_by=policy_name.asc&rule_name.asc" % (
    ...
    def get_csp_findings(self, page):
        url = "%s:%d/%s/csp_findings?sort_by=policy_name.asc&rule_name.asc&per_page=100&page=%s" % (
