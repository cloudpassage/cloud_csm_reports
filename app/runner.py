...
def main():
    utility.Utility.log_stdout("Cloud CSM Reports Script Started ...")
    ...
    absolute_path, file_name, current_time = json_to_csv_obj.prepare_csv_file(output_directory)
    ...
    for page in range(pages):
        ...
        total_pass_count, total_count, total_critical_fail_count, total_non_critical_fail_count, total_fail_count = json_to_csv_obj.prepare_report_statistics(csp_findings_lst)
    ...
    overall_pass = round(((overall_total_pass_count/overall_total_count)*100), 2)
    overall_critical_fail = round(((overall_total_critical_fail_count/overall_total_count)*100), 2)
    overall_non_critical_fail = round(((overall_total_non_critical_fail_count/overall_total_count)*100), 2)
    ...
    for page in range(pages):
        ...
        json_to_csv_obj.convert_json_to_csv(counter, absolute_path, file_name, current_time, csp_findings_lst, overall_pass, overall_critical_fail, overall_non_critical_fail)

    """
    Operation Completed
    """
    utility.Utility.log_stdout(
        "5- Operation Completed, Check Generated CSV File!")
